...
class TestLoginCourier:
    ...
    message_400 = 'Недостаточно данных для входа'
    message_404 = 'Учетная запись не найдена'
    ...
    def test_successful_login_code200(self, register_new_courier):
    ...
    def test_login_without_login_code400(self, register_new_courier):
        login_pass = register_new_courier
        payload_login = {"login": "",
            "password": login_pass[1]}
        response = requests.post(URL_LOGIN, data=payload_login)
    ...
    def test_login_without_password_code400(self, register_new_courier):
        login_pass = register_new_courier
    ...
    def test_login_with_unregistered_login_code404(self, request_body):
        ...
        assert response.json()['message'] == self.message_404
