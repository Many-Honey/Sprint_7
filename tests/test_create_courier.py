...
class TestCreateCourier:
    ...
    def test_successful_creation_code201(self, generate_request_body_and_delete_courier):
        payload = generate_request_body_and_delete_courier
    ...
    def test_create_courier_without_login_code400(self, generate_request_body_and_delete_courier):
        payload = generate_request_body_and_delete_courier
        payload["login"] = ""
        response = requests.post(URL_COURIER, data=payload)
    ...
    def test_create_courier_without_password_code400(self, generate_request_body_and_delete_courier):
        payload = generate_request_body_and_delete_courier
        payload["password"] = ""
        response = requests.post(URL_COURIER, data=payload)
        assert response.status_code == 400
    ...
    def test_create_two_couriers_with_same_login_code409(self, generate_request_body_and_delete_courier):
        payload = generate_request_body_and_delete_courier
        response_1 = requests.post(URL_COURIER, data=payload)
        assert response_1.status_code == 201
        response_2 = requests.post(URL_COURIER, data=payload)
        assert response_2.status_code == 409
        assert response_2.json() == {'code': 409, 'message': 'Этот логин уже используется. Попробуйте другой.'}
